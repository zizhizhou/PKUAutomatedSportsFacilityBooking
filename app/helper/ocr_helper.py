...
class Ddddocr:
    ...
    def get_img_xy(self, img):
        click_identify_result = self.ddddocr_click_identify(img, (0, 0, 310, 155))
        img_xy = {}
        for key, xy in click_identify_result.items():
            if self.is_chinese_char(key):
    ...
    def ddddocr_click_identify(self, content, crop_size=None):
        img = Image.open(BytesIO(content))
        img = img.resize((310, 155), Image.ANTIALIAS)
        if crop_size:
    ...
    def is_chinese_char(self,char):#判断是否为单个中文汉字
        if len(char)!=1:
            return False
        else:
            return '\u4e00' <= char <= '\u9fa5'
