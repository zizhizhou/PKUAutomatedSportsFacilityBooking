...
from app.db.models import Base,UserAction
...
if Config._wechat_notice:
    _Engine = create_engine(
        ...
        f"sqlite:///{os.path.join(Config().get_db_path(), 'user_wechat.db')}?check_same_thread=False",
        echo=False,
        poolclass=QueuePool,
        pool_pre_ping=True,
        pool_size=100,
    ...
    _Engine = create_engine(
        ...
        f"sqlite:///{os.path.join(Config().get_db_path(), 'user_local.db')}?check_same_thread=False",
        echo=False,
        poolclass=QueuePool,
        pool_pre_ping=True,
        pool_size=100,
...
class MainDb:
    ...
    def init_data(self):
        ...
        config_dir = Config().get_sqls_path()
    ...
    def excute(self, sql, params=None):
        ...
        self.session.execute(compiled_sql)
