...
def check_config():
    ...
    if Config().get_config('app'):
        ...
        if logtype:
            logger.info(f"日志输出类型为：{logtype}")
        if logtype == "server":
            logserver = Config().get_config('app').get('logserver')
            if not logserver:
                logger.warn("【Config】日志中心地址未配置，无法正常输出日志")
            else:
                logger.info("日志将上送到服务器：{logserver}")
        elif logtype == "file":
            logpath = Config().get_config('app').get('logpath')
            if not logpath:
                logger.warn("【Config】日志文件路径未配置，无法正常输出日志")
            else:
        ...
        if not web_port:
            logger.warn("【Config】WEB服务端口未设置，将使用默认3000端口")

        # 检查登录用户和密码
        login_user = Config().get_config('app').get('login_user')
        login_password = Config().get_config('app').get('login_password')
        if not login_user or not login_password:
            logger.warn("【Config】WEB管理用户或密码未设置，将使用默认用户：admin，密码：password")
        else:
            logger.info(f"WEB管理页面用户：{str(login_user)}")

        # 检查HTTPS
        ssl_cert = Config().get_config('app').get('ssl_cert')
        ssl_key = Config().get_config('app').get('ssl_key')
        if not ssl_cert or not ssl_key:
            logger.info(f"未启用https，请使用 http://IP:{str(web_port)} 访问管理页面")
        else:
            if not os.path.exists(ssl_cert):
                logger.warn(f"【Config】ssl_cert文件不存在：{ssl_cert}")
            if not os.path.exists(ssl_key):
                logger.warn(f"【Config】ssl_key文件不存在：{ssl_key}")
            logger.info(f"已启用https，请使用 https://IP:{str(web_port)} 访问管理页面")
    else:
        logger.error("【Config】配置文件格式错误，找不到app配置项！")
...
class ConfigMonitor(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        ...
        if file_name == "config.yaml" :
            ...
            logger.warn(f"【System】进程 {os.getpid()} 检测到系统配置文件已修改，正在重新加载...")
            time.sleep(1)
            # 重新加载配置
            Config().init_config()
            # 重载singleton服务
